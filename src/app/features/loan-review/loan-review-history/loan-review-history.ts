...
export class LoanReviewHistory implements OnInit {
  ...
  fetchHistory(): void {
    ...
    this.loanApplicationService
      .getLoanReviewHistory(this.currentPage, this.pageSize)
      .subscribe({
        next: (response) => {
        ...
        error: (err) => {
          console.error('Failed to fetch loan review history:', err);
          this.allHistories = [];
