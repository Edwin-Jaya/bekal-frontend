...
export class LoanApprovals {
  private http = inject(HttpClient);
  private apiUrl = `${environment.apiUrl}/loan-applications`;

  getPendingLoanApplicationApproval(page: number = 1, size: number = 10): Observable<any> {
  ...
  submitApproval(payload: SubmitPayload): Observable<LoanApprovalResponse> {
    return this.http.post<LoanApprovalResponse>(`${environment.apiUrl}/loan-approval/submit`, payload, {withCredentials:true});
  }

  getApprovalDetail(loanId: string): Observable<LoanApprovalDetailInterface> {
    return this.http.get<LoanApprovalDetailInterface>(`${this.apiUrl}/${loanId}/detail-approval`,{withCredentials:true});
  }
  ...
  getLoanApprovalHistory(page: number = 0, size: number = 10): Observable<PageResponse<LoanHistoryResponse>> {
    ...
    return this.http.get<PageResponse<LoanHistoryResponse>>(`${environment.apiUrl}/loan-approval/history`, { params, withCredentials:true});
  }
}
