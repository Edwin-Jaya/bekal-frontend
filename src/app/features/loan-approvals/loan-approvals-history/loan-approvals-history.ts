...
import { LoanApprovals } from '../service/loan-approvals';
...
@Component({
  selector: 'app-loan-approvals-history',
  imports: [ReviewHistoryTable,Pagination,CommonModule],
  templateUrl: './loan-approvals-history.html',
  styleUrl: './loan-approvals-history.css',
})
export class LoanApprovalsHistory {
  private loanApplicationService = inject(LoanApprovals);
  ...
  fetchHistory(): void {
    ...
    this.loanApplicationService.getLoanApprovalHistory(this.currentPage, 10).subscribe({
      next: (response) => {
