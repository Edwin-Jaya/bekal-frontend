...
export class LoanApprovalDetail implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (activeId) {
      ...
      console.error('No approval ID found in route URL');
      this.isLoading = false;
      this.feedbackService.show({
        type: 'error',
        title: 'ID Tidak Ditemukan',
        message: 'URL tidak valid atau ID persetujuan tidak ditemukan.',
        onConfirm: () => this.back(),
      });
    ...
    this.loanService.getApprovalDetail(this.id).subscribe({
      ...
      error: (err) => {
        ...
        this.feedbackService.show({
          ...
          message: 'Terjadi kesalahan saat mengambil detail pengajuan loan.',
        });
        this.cdr.markForCheck();
      },
    });
    ...
    this.loanService.submitApproval(payload).subscribe({
      next: () => {
        ...
        this.feedbackService.show({
          type: 'success',
          title: 'Berhasil',
          message: 'Hasil peninjauan loan berhasil dikirim!',
          confirmText: 'Selesai',
          onConfirm: () => {
            this.router.navigate(['branch-manager/loan-approvals']);
          },
        });
      },
      error: (err) => {
